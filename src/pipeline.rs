...
/// Turn one freshly-detected post into a scored candidate in the pool.
/// Called only for URNs we've never seen (dedup happens in the crawl loop).
pub async fn ingest(state: &AppState, post: RawPost) -> anyhow::Result<()> {
    ...
    let (draft_subject, draft_body) = if matches!(tier, Tier::Exceptional | Tier::Strong) {
    ...
    db::insert_candidate(&state.pool, &nc).await?;

    tracing::info!(
        tier = tier.as_str(), score, company = %post.company,
        "scored: {}", post.title
    );
    Ok(())
}
